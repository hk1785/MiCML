      ...
      tabItems(
        ...
        tabItem(tabName = "home",
                div(id = "homepage", br(), HOME_COMMENT_MV, HOME_COMMENT, br(),
                    div(tags$img(src="Home2.png", height = 175, width = 540), style = "text-align: center;"), "https://xkcd.com/552/", br(),
...
server = function(input, output, session){
  ...
  otu.tab <- read.table(file = "Data/otu.tab.txt", header = TRUE, check.names = FALSE, sep = "\t")
  tax.tab <- read.table(file = "Data/tax.tab.txt", header = TRUE, check.names = FALSE, sep = "\t")
  sam.dat <- read.table(file = "Data/sam.dat.txt", header = TRUE, check.names = FALSE, sep = "\t")
  tree <- read.tree(file = "Data/tree.tre")
  
  output$downloadData.Immuno <- downloadHandler(
    filename = function() {
      paste("Immuno.Metagenome.Char.Rdata", sep = "")
    },
  ...
  output$downloadZip.Immuno <- downloadHandler(
    filename = function() {
      paste("Immuno.Metagenome.Char",".zip", sep = "")
    },
  ...
  output$cf_method <- renderUI({
    tagList(
      ...
      # tags$style(type = 'text/css', "#cf_method .pretty .state label::before {margin-top: 20px}"),
      # tags$style(type = 'text/css', "#cf_method .icon {margin-top: 20px}"),
      # tags$style(type = 'text/css', "#cf_method .pretty .state label {margin-left: 20px}")
    )
  })
