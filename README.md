...
* **Data** - In this directory, example microbiome data are stored in a widely used unified format, called phyloseq (see the file: biom.Rdata in Data/Phyloseq) as well as individual files (see the files: feature table (otu.tab.txt), taxonomic table (tax.tab.txt), and metadata/sample information (sam.dat.txt) in Data/Phyloseq).
